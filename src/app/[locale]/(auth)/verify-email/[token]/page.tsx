...
// Helper function to decode JWT token (without verification, just for extracting email)
function decodeJWTToken(token: string): { email?: string } | null {
  try {
    ...
    if (decoded.data && decoded.data.email) {
      return { email: decoded.data.email };
    }
          ...
          {userEmail && (
            <>
              ...
              <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">
                {language === 'id'
                  ? `Link verifikasi akan dikirim ke ${userEmail}`
                  : `Verification link will be sent to ${userEmail}`}
              </p>
            </>
          )}
