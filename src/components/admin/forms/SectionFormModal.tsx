'use client';

import React, { useEffect, useState } from 'react';
import { AdminModal } from '@/components/admin/ui';
import { AdminSection } from '@/types/admin';
import { createSection, updateSection, getProducts } from '@/lib/adminApi';
import { AdminProduct } from '@/types/admin';
import { Loader2 } from 'lucide-react';

interface SectionFormState {
  code: string;
  title: string;
  icon: string;
  order: string;
  isActive: boolean;
  productsInput: string;
}

const defaultSectionState: SectionFormState = {
  code: '',
  title: '',
  icon: '⭐',
  order: '1',
  isActive: true,
  productsInput: '',
};

const parseCommaValues = (value: string) =>
  value
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);

interface SectionFormModalProps {
  open: boolean;
  onClose: () => void;
  initialData?: AdminSection | null;
  onSuccess?: () => void;
}

export default function SectionFormModal({
  open,
  onClose,
  initialData,
  onSuccess,
}: SectionFormModalProps) {
  const [form, setForm] = useState<SectionFormState>(defaultSectionState);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [products, setProducts] = useState<AdminProduct[]>([]);
  const [loadingProducts, setLoadingProducts] = useState(false);

  useEffect(() => {
    if (open) {
      if (initialData) {
        setForm({
          code: initialData.code || '',
          title: initialData.title || '',
          icon: initialData.icon || '⭐',
          order: String(initialData.order || 1),
          isActive: initialData.isActive,
          productsInput: initialData.products?.join(', ') || '',
        });
      } else {
        setForm(defaultSectionState);
      }
      setError('');

      // Load products for autocomplete
      loadProducts();
    }
  }, [open, initialData]);

  const loadProducts = async () => {
    try {
      setLoadingProducts(true);
      const data = await getProducts({ limit: 100 });
      setProducts(data.products || []);
    } catch (err) {
      console.error('Failed to load products', err);
    } finally {
      setLoadingProducts(false);
    }
  };

  const handleChange = (key: keyof SectionFormState, value: string | boolean) => {
    setForm((prev) => ({
      ...prev,
      [key]: value,
    }));
  };

  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setLoading(true);
    setError('');

    try {
      const productsArray = parseCommaValues(form.productsInput);

      const payload: any = {
        code: form.code.trim(),
        title: form.title.trim(),
        icon: form.icon.trim() || '⭐',
        order: parseInt(form.order) || 1,
        isActive: form.isActive,
        products: productsArray,
      };

      if (initialData) {
        await updateSection(initialData.id, payload);
      } else {
        await createSection(payload);
      }

      onSuccess?.();
      onClose();
    } catch (err: any) {
      setError(err.message || 'Gagal menyimpan section');
    } finally {
      setLoading(false);
    }
  };

  const commonIcons = ['⭐', '⚡', '🎁', '📅', '🔥', '💎', '🏆', '🎯', '⚽', '🎮'];

  return (
    <AdminModal
      open={open}
      onClose={loading ? () => null : onClose}
      title={initialData ? 'Edit Section' : 'Tambah Section'}
      description="Sesuaikan data section untuk mengelompokkan SKU"
      maxWidthClass="max-w-2xl"
      footer={
        <div className="flex items-center justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            disabled={loading}
            className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-900"
          >
            Batal
          </button>
          <button
            type="submit"
            form="section-form"
            disabled={loading}
            className="px-4 py-2 rounded-lg bg-primary text-white text-sm font-semibold hover:bg-primary/90 disabled:opacity-60"
          >
            {loading ? 'Menyimpan...' : 'Simpan'}
          </button>
        </div>
      }
    >
      <form id="section-form" onSubmit={handleSubmit} className="space-y-4">
        {error && (
          <div className="p-3 rounded-lg bg-red-50 text-sm text-red-700">{error}</div>
        )}

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              Kode Section <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={form.code}
              onChange={(e) => handleChange('code', e.target.value)}
              placeholder="special-item"
              required
              disabled={!!initialData}
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent disabled:bg-gray-50 disabled:text-gray-500"
            />
            <p className="text-xs text-gray-500 mt-1">Kode unik untuk section (huruf kecil, tanpa spasi)</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              Urutan <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              value={form.order}
              onChange={(e) => handleChange('order', e.target.value)}
              placeholder="1"
              required
              min="1"
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
            />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Nama Section <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            value={form.title}
            onChange={(e) => handleChange('title', e.target.value)}
            placeholder="Spesial Item"
            required
            className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Icon <span className="text-red-500">*</span>
          </label>
          <div className="space-y-2">
            <input
              type="text"
              value={form.icon}
              onChange={(e) => handleChange('icon', e.target.value)}
              placeholder="⭐"
              maxLength={2}
              required
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent text-center text-2xl"
            />
            <div className="flex flex-wrap gap-2">
              {commonIcons.map((icon) => (
                <button
                  key={icon}
                  type="button"
                  onClick={() => handleChange('icon', icon)}
                  className={`w-10 h-10 rounded-lg border-2 flex items-center justify-center text-xl transition-colors ${
                    form.icon === icon
                      ? 'border-primary bg-primary/10'
                      : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
                  {icon}
                </button>
              ))}
            </div>
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Product Codes
          </label>
          <input
            type="text"
            value={form.productsInput}
            onChange={(e) => handleChange('productsInput', e.target.value)}
            placeholder="MLBB, FF, PUBGM"
            className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
          />
          <p className="text-xs text-gray-500 mt-1">
            Pisahkan dengan koma (contoh: MLBB, FF, PUBGM). Kosongkan untuk semua produk.
          </p>
          {loadingProducts ? (
            <div className="mt-2 flex items-center gap-2 text-sm text-gray-500">
              <Loader2 className="w-4 h-4 animate-spin" />
              Memuat produk...
            </div>
          ) : products.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2">
              {products.slice(0, 10).map((product) => (
                <button
                  key={product.id}
                  type="button"
                  onClick={() => {
                    const currentProducts = parseCommaValues(form.productsInput);
                    if (currentProducts.includes(product.code)) {
                      handleChange(
                        'productsInput',
                        currentProducts.filter((p) => p !== product.code).join(', ')
                      );
                    } else {
                      handleChange('productsInput', [...currentProducts, product.code].join(', '));
                    }
                  }}
                  className={`px-2 py-1 text-xs rounded border transition-colors ${
                    parseCommaValues(form.productsInput).includes(product.code)
                      ? 'bg-primary text-white border-primary'
                      : 'bg-gray-50 text-gray-700 border-gray-200 hover:bg-gray-100'
                  }`}
                >
                  {product.code}
                </button>
              ))}
            </div>
          )}
        </div>

        <div className="flex items-center gap-2">
          <input
            type="checkbox"
            id="isActive"
            checked={form.isActive}
            onChange={(e) => handleChange('isActive', e.target.checked)}
            className="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary"
          />
          <label htmlFor="isActive" className="text-sm font-medium text-gray-700">
            Section Aktif
          </label>
        </div>
      </form>
    </AdminModal>
  );
}

