...
export function StepSection({
    ...
    return (
        <section
            className={cn(
                'relative rounded-xl md:rounded-2xl transition-all duration-300 border', // Removed overflow-hidden
                disabled
        ...
        >
            ...
            <div
                className={cn(
                    'px-4 py-3 md:px-6 md:py-4 border-b border-gray-100 dark:border-gray-700',
                    'bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-800/50',
                    'rounded-t-xl md:rounded-t-2xl' // Manually round top corners
                )}
            >
                <h2
                    className={cn(
                        'text-base md:text-lg font-bold flex items-center gap-2 md:gap-3',
                        disabled ? 'text-gray-400' : 'text-gray-900 dark:text-white'
                    )}
                >
                    <div
                        className={cn(
                            'w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs md:text-sm font-bold transition-all shrink-0',
                            isCompleted
            ...
            {/* Content */}
            <div className={cn(
                'transition-all p-3 md:p-6', // Added padding here for content
                disabled && 'blur-sm select-none'
            )}>
                {children}
            </div>
